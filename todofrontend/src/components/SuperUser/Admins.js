...
const Admins = () => {
    ...
    const filteredAdmin= admins.filter((admin) => {
        ...
        return (
          (admin.username && admin.username.toLowerCase().includes(lowerSearchTerm)) ||
          (admin.email && admin.email.toLowerCase().includes(lowerSearchTerm)) ||
          (admin.employeeId && admin.employeeId.toLowerCase().includes(lowerSearchTerm))
        );
      });
    return (
        <div>
            ...
            {filteredAdmin.length > 0 ? (
                <Table striped bordered hover className=" mt-4 text-center border border-dark" style={{ borderRadius: '10px !important' }}>
                    <thead>
                        <tr>
